...
async function initDB() {
  try {
    ...
    const coursesData = [
      { module: 'Microeconomics', content: microData, notes_url: 'assets/notes/micro.pdf' },
      { module: 'Macroeconomics', content: macroData, notes_url: 'assets/notes/macro.pdf' },
      { module: 'Econometrics', content: econometricsData, notes_url: 'assets/notes/econometrics.pdf' },
      { module: 'Development Economics', content: developmentData, notes_url: 'assets/notes/development.pdf' },
      { module: 'Finance', content: financeData, notes_url: 'assets/notes/finance.pdf' }
    ];

    for (const data of coursesData) {
      try {
        const query = `
          INSERT INTO courses (module, content, notes_url) 
          VALUES ($1, $2, $3) 
          ON CONFLICT (id) DO NOTHING;
        `;
        await pool.query(query, [data.module, data.content, data.notes_url]);
      } catch (err) {
        console.error(`Error inserting course ${data.module}:`, err);
      }
    }
